    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className="inline-flex items-center gap-1 rounded-full border border-primary/20 bg-primary/5 px-2.5 py-0.5 text-[13px] font-medium text-primary hover:bg-primary/10 hover:border-primary/30 transition-all duration-200 cursor-pointer shadow-sm"
    ...
    return (
      <div className={`flex justify-end ${isGrouped ? "mt-0.5" : ""}`}>
        <motion.div
          ...
          className={`relative max-w-[75%] rounded-2xl px-3.5 py-2.5 shadow-sm bg-primary text-primary-foreground shadow-primary/10 rounded-br-[4px] ${isGrouped ? "rounded-br-2xl" : ""}`}
        >
          {!isGrouped && (
            <svg className="absolute -right-[5px] bottom-0 w-[10px] h-[16px]" viewBox="0 0 10 16" fill="none">
              <path d="M0 16H5C7.76 16 10 13.76 10 11V0C10 5 7 10 0 12V16Z" fill="hsl(var(--primary))" />
            </svg>
          )}
  ...
  return (
    <div className={`flex flex-col items-start ${isGrouped ? "mt-0.5" : ""}`}>
      ...
      <motion.div
        ...
        animate={{ scale: 1, opacity: 1, y: 0 }}
        transition={{ type: "spring", stiffness: 400, damping: 28 }}
        className={`relative max-w-[85%] rounded-2xl px-3.5 py-2.5 shadow-sm bg-white/[0.04] border border-white/[0.08] ${!isGrouped ? "rounded-tl-[4px]" : ""}`}
      >
        {/* Tail pointing up-left toward Cosmii */}
        {!isGrouped && (
          <svg className="absolute -top-[5px] left-[10px] w-[12px] h-[7px]" viewBox="0 0 12 7" fill="none">
            <path d="M0 7H12C8.5 7 4 3.5 2 0C2 2.5 1.5 5 0 7Z" fill="rgba(255,255,255,0.04)" />
            <path d="M2 0C2 2.5 1.5 5 0 7" stroke="rgba(255,255,255,0.08)" strokeWidth="1" fill="none" />
          </svg>
        )}
        ...
        <div className="text-[14px] leading-relaxed text-white/80 prose prose-sm prose-invert max-w-none
            ...
            prose-blockquote:border-indigo-400/30 prose-blockquote:text-white/50 prose-blockquote:not-italic
